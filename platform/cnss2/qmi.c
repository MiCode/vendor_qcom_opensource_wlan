...
#define ELF_BDF_FILE_NAME_M2		"bd_m2.elf"
#define ELF_BDF_FILE_NAME_M2_GLOBAL	"bd_m2gl.elf"
#define ELF_BDF_FILE_NAME_M3		"bd_m3.elf"
#define ELF_BDF_FILE_NAME_M3_GLOBAL	"bd_m3gl.elf"
#define ELF_BDF_FILE_NAME_M11           "bd_m11.elf"
#define ELF_BDF_FILE_NAME_M11_GLOBAL    "bd_m11gl.elf"
	...
	case CNSS_BDF_ELF:
		...
		if (plat_priv->board_info.board_id == 0xFF) {
			if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK)
				...
				if (hw_platform_ver == HARDWARE_PROJECT_M2) {
					if ((uint32_t)CountryGlobal == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_M2_GLOBAL);
					else
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_M2);
				} else if (hw_platform_ver == HARDWARE_PROJECT_M3) {
					if ((uint32_t)CountryGlobal == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_M3_GLOBAL);
					else
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_M3);
				} else if (hw_platform_ver == HARDWARE_PROJECT_M11) {
                                        if ((uint32_t)CountryGlobal == hw_country_ver)
                                        snprintf(filename_tmp, filename_len,
                                                ELF_BDF_FILE_NAME_M11_GLOBAL);
                                        else
                                        snprintf(filename_tmp, filename_len,
                                                ELF_BDF_FILE_NAME_M11);
                                }
				else
				snprintf(filename_tmp, filename_len,
					 ELF_BDF_FILE_NAME);
